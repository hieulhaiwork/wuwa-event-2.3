...
CUBE_NUM = 6
CUBE_NAMES = ['changli','shore','camellya','carlotta','anh_lao_cong','jinhsi'] # Adjust the order based on reality from behind to front
BOARD_LENGTH = 23 + 4
...
def simulate(verbose=True):
    ...
    for cube in cubes:
        if cube.name == 'changli':
            ...
            cube.position = 0
        elif cube.name == 'shore':
            board_state[1].append(cube)
            cube.position = 1
        elif cube.name == 'camellya':
            board_state[1].append(cube)
            cube.position = 1
        elif cube.name == 'carlotta':
            board_state[2].append(cube)
            cube.position = 2
        elif cube.name == 'anh_lao_cong':
            board_state[2].append(cube)
            cube.position = 2
        elif cube.name == 'jinhsi':
            board_state[3].append(cube)
            cube.position = 3
    
    ## Jinhsi's position in the game coincides with the finish pad, so from this position you have to go 24 more squares to reach the destination.
